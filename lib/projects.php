...
class OC_Collaboration_Project
{
	...
	public static function createProject($title, $desc, $creator, $deadline, $details=NULL)
	{
		...
		try
		{
			...
			$calendar_id = 0;//OC_Calendar_Calendar::addCalendar(\OC_User::getUser(), $title, 'VEVENT,VTODO,VJOURNAL', null, 0, '#3a87ad');
			...
			if($cnt != 0 && isset($details[0]['member']))
			{
				foreach($details as $detail)
				{
					...
					\OC::$server->getConfig()->setUserValue($member, 'settings', 'email', $detail['email']);
					\OC::$server->getConfig()->setUserValue($member, 'collaboration', 'mobile', $detail['mobile']);
				}
	...
	public static function updateProject($pid, $title, $desc, $deadline, $details, $updater, $completed)
	{
		...
		try
		{
			...
			$query->execute(array($title, $desc, OC_Collaboration_Time::convertUITimeToDBTime($deadline . ' 23:59:59'), intval($completed), $pid));
